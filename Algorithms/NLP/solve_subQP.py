...
def create_modified_QP(Bk,Jac_fk,gk,Jac_gk, x0 = None):
    ...
    At[:,n_var:n_var+n_eq] = np.eye(n_eq)
    At[:,n_var+n_eq:n_var+2*n_eq] = (-1) * np.eye(n_eq)
    #At[:,:n_var] = gradient of equality constraints transpose evaluated at xk
    A = At.T

    b = np.zeros(n_eq)
    # b = (-1) * equality constraint evaluated at xk    
